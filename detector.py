    ...
    if config.get("detector", "snowboy") == "porcupine":
        ...
        if keyword_paths:
            porcupine = pvporcupine.create(
                ...
                model_path="/home/pi/.wukong/models/porcupine_params_zh.pv",        # 增加了中文模型路径
        ...
        recorder = PvRecorder(device_index=1, frame_length=porcupine.frame_length) # 修改了device_index,由-1(系统默认)改为了1(通过下面的 get_audio_devices() 进行选择)
        ...
        try:
            while True:
                ...
                if result >= 0:
                    ...
                    if random_number == 1:
                        wukong.conversation.say(f"我在")
                    elif random_number == 2:
                        wukong.conversation.say(f"诶！有什么问题请问我吧！")
                    elif random_number == 3:
                        wukong.conversation.say(f"你好")
                    
                    wukong._detected_callback(kw)
                    # wukong.conversation.interrupt()
                    query = wukong.conversation.activeListen()
                    # 增加简单判断
                    if query:
                        wukong.conversation.say(f"我想一下")
