...
# from robot.detector import recorder
...
class Conversation(object):
    ...
    def doConverse(self, fp, callback=None, onSay=None, onStream=None):
        self.interrupt()
        logger.info(f"开始识别音频文件：{fp}(存在：{os.path.exists(fp)})")  # 新增日志
        try:
            query = self.asr.transcribe(fp)
            logger.info(f"ASR识别结果：{query}")  #新增日志
        except Exception as e:
    ...
    def activeListen(self, silent=False):
        ...
        # :param
        #   基于 Porcupine 的 PvRecorder 进行录制音频
        """
        if self.immersiveMode: # 初始为None
            self.player.stop()  # 来自 Player.py 这里用于确保在进入主动聆听之前强制停止当前播放的音频
        elif self.player.is_playing(): # 如果当前有音频正在播放
            self.player.join()  # 确保所有音频都播完
        logger.info("进入主动聆听...")
        try:
            if not silent:
                self.lifeCycleHandler.onWakeup()    # LifeCycleHandler.py   onwakeup()触发唤醒生命周期事件
            
            # time.sleep(1.5)     # 延时1.5秒，等待音箱播放结束
            # 使用系统录音设备录音，注意 plughw:x,y  需要在开机后查看麦克风设备的编号进行修改
            voice = '/home/pi/wukong-robot/temp/active_listen.wav'
            logger.info(f"开始录音-->")
            # self.say(f"我在")
            os.system(f"arecord -D plughw:3,0 -f S16_LE -d 7 {voice}")
            logger.info(f"录音结束<--end")        
            
            logger.info(f"voice_path:{voice}")
            ...
            # 录制的音频进行有效判断
            # 2025.8.6  当前ASR文字识别没有问题，音频录制有问题
            if voice:
                query = self.asr.transcribe(voice)  # 调用语音识别引擎，详见 ASR.py，反馈识别文本 
                # utils.check_and_delete(voice)   # 有效则将文件删除
                return query
            else:
                logger.error(f"音频录制无效")
            return ""
            
        except Exception as e:
            logger.error(f"主动聆听失败：{e}", stack_info=True)
            traceback.print_exc()       # 异常堆栈日志
            return ""
