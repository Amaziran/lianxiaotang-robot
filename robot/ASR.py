...
class TencentASR(AbstractASR):
    ...
    def __init__(self, appid, secretid, secret_key, region="ap-beijing", **args):
        super(self.__class__, self).__init__()
        self.engine = TencentSpeech.tencentSpeech(secret_key, secretid)         # tencentspeech来自 /sdk/TencentSpeech.py
        self.region = region
        self.appid = appid  # 存储appid用于生成临时文件
    ...
    def transcribe(self, fp):           # fp 是来自Conversation.py的query = self.asr.transcribe(voice)传参，为voice
        mp3_path = utils.convert_wav_to_mp3(fp)     # 格式转换 wav -> mp3
        # logger.info(f"map3_path:{mp3_path}")
        r = self.engine.ASR(mp3_path, "mp3", "1", self.region)      # 调用腾讯云API
